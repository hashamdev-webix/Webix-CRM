...
export const GET = withPermission('leads.dataentry.view', async (req, { params }) => {
  ...
  const [total, remarks] = await Promise.all([
    LeadRemark.countDocuments({ lead_id: params.id, lead_type: 'dataentry' }),
    LeadRemark.find({ lead_id: params.id, lead_type: 'dataentry' })
...
export const POST = withPermission('leads.dataentry.edit', async (req, { params }, session) => {
  await connectDB();
  const { remark_text } = await req.json();
  if (!remark_text?.trim()) return NextResponse.json({ error: 'remark_text is required' }, { status: 400 });
